...
# LOCAL PATCH (2026-07-30, rebased onto 3.1.2 on 2026-09-02). Upstream 3.1.0
# absorbed the skip-if-text gate (documents.consumer.should_produce_archive
# now decides, and parse() honours produce_archive). What stays local is the
# operator escape hatch: while this file exists AND the deadline inside it is
# in the future, a PDF the consumer classified as born digital still goes to
# the remote engine, with its old text layer stripped first. For re-OCRing
# documents whose existing text layer is bad.
#
...
def _force_all_applies(path: Path) -> bool:
    """Return True when the force-all window overrides the born-digital skip.

    Upstream (since 3.1.0) leaves a PDF that the consumer classified as born
    digital to local text extraction. Force-all exists precisely to re-OCR
    documents whose existing text layer is bad, so while the window is open
    such a document still goes to the engine, after the strip step in
    ``parse()``. Vector-text documents are never sent: there is no bad OCR
    in those to replace, their embedded text beats any OCR of a rendered
    page, and stripping them would blank them.
    """
    ...
    if total and len(visible) >= total:
        logger.info(
            "Remote OCR skipped even under force-all: every page of %s draws "
            "visible text, so it is born digital and OCR cannot improve it",
            path.name,
        )
        return False
    return True
...
class RemoteDocumentParser:
    ...
    def parse(
        ...
        *,
        ...
        produce_archive:
            ...
            remote engine and uses locally-extracted text instead, unless
            the force-all window is open (LOCAL PATCH, see below).
        """
        config = RemoteEngineConfig.from_app_config()
        ...
        if (
            ...
            and not _force_all_applies(document_path)
        ):
            logger.debug(
    ...
    def _azure_ai_vision_parse(
        ...
        *,
        ...
        Downloads the searchable PDF output from Azure, recompresses it
        the way the local pipeline would (LOCAL PATCH) and stores it at
        ``self._archive_path``.
    ...
    def _optimize_archive(self, raw: Path) -> Path:
        ...
        LOCAL PATCH (2026-07-30) — upstream discussion #13264; still not
        upstream as of 3.1.2.  Upstream stores the engine's PDF verbatim, so it bypasses
    ...
    def _stamp_provenance(self, path: Path) -> Path:
        ...
        engine = RemoteEngineConfig.from_app_config().engine or "remote"
        stamp = (
